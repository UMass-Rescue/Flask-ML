...
class TestResponseModel(unittest.TestCase):
    def test_valid_response_with_file_results(self):
        data = {
            ...
            "results": BatchImageResult(
                results=[
                    ImageResult(**{"id": "file1.txt", "result": "output_file1.png"}),
                    ImageResult(**{"id": "file2.txt", "result": "output_file2.png"}),
                ]
            ),
        }
    ...
    def test_valid_response_with_missing_status(self):
        data = {"results": {"results": [{"id": "file1.txt", "result": "output_file1.txt"}]}}
        response = ResponseModel(**data)
        self.assertEqual(response.status, "SUCCESS")
    ...
    def test_get_response_custom_status_code(self):
        response_model = ResponseModel(
            status="SUCCESS",
            results=BatchTextResult(results=[TextResult(result="Processed text", id="Sample text")]),
        )
